...
void AimAtPlayers()
{
	...
	for (int i = 1; i < g_pGlobals->maxClients; ++i)
	{
		...
		Vector pos;
		if (!target || target->IsDormant() || target->GetHealth() < 1)
			continue;
...
void CAntiaim::Run(QAngle org_view)
{
	if (RagebotElements.Ragebot_AntiAim_Active->Checked)
	{
		...
		else
		{
			...
			{
				if (G::MainWeapon->IsMiscWeapon() && RagebotElements.Ragebot_AntiAim_Disable_Knife && RagebotElements.Ragebot_AntiAim_Disable_Knife->Checked)
					return;
			}
			...
			if (RagebotElements.Ragebot_AntiAim_Edge && RagebotElements.Ragebot_AntiAim_Edge->SelectedIndex > 0)
			{
			...
			if (RagebotElements.Ragebot_AntiAim_FreeStand && RagebotElements.Ragebot_AntiAim_FreeStand->SelectedIndex > 0)
			{
			...
			if (RagebotElements.Ragebot_AntiAim_AtTargets && RagebotElements.Ragebot_AntiAim_AtTargets->SelectedIndex > 0)
			{
				AimAtPlayers();
